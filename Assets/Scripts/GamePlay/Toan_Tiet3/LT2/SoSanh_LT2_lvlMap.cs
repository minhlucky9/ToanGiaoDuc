...
public class SoSanh_LT2_lvlMap : MonoBehaviour, LevelMap
{
    public SS_ItemManageLT2 itemManager;
    public SS_AreaReceiverManagerLT2 areaManage;
    ...
    public void OnFinishLevel()
    {
        ...
        //calculate
        List<SS_AreaReceiverLT2> listAreaToCheck = areaManage.GetList();
        for (int i = 0; i < listAreaToCheck.Count; i++)
        {
            if (listAreaToCheck[i].IsWinThisArea())
            {
                rightCount++;
            }
            else
    ...
    public void OnQuitLevel()
    {
        ...
        GetComponentInChildren<SS_RandomLT2>().StopAllCoroutines();
        StopAllCoroutines();
