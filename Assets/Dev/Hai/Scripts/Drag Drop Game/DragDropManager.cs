...
namespace DragDrop{
    public class DragDropManager : MonoBehaviour
    {
        ...
        public void CorrectWork()
        {
            if (hasShownAnswer)
            {
                foreach (GameObject dropZoneObject in dropZones)
                {
                    DropZone dropZone = dropZoneObject.GetComponent<DropZone>();
                    StartCoroutine(dropZone.ShowAnswer());
                }
            } else
            {
                Debug.Log("Đã chữa bài");
            }
